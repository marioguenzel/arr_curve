    ...
    for itask in PSet:
        if i < numV:
            ...
            for j in range(numpaths):
                ...
                if j != iMaxE:
                    path["Cseg"] = seg_UUniFast(
                        maxnumsegs, itask['execution']*random.uniform(scalef, 1))
                else:
                    path["Cseg"] = seg_UUniFast(maxnumsegs, itask['execution'])

                if j != iMaxS:
                    path["Sseg"] = seg_UUniFast(
                        maxnumsegs-1, itask['sslength']*random.uniform(scalef, 1))
                else:
                    path["Sseg"] = seg_UUniFast(
                        maxnumsegs-1, itask['sslength'])
...
def taskGeneration_p(numTasks, uTotal, minsslength, maxsslength, Pmin=100, numLog=1, vRatio=1, seed=1, numsegs=2, minSratio=1, numpaths=2, scalef=0.8):
    ...
    SSS_seg_gen(vRatio, minsslength, maxsslength,
                numsegs, minSratio, numpaths, scalef)
    return PSet
